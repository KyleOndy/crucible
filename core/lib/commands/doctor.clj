...
(defn- check-system-environment
  ...
  (let [java-version (System/getProperty "java.version")
    ...
    [{:level :ok :category "System" :message (str "Operating System " os-name " " os-version)}
     {:level :ok :category "System" :message (str "Java Version " java-version)}
     {:level :ok :category "System" :message (str "Babashka Version " bb-version)}
     {:level :ok :category "System" :message (str "Working Directory " working-dir)}]))
...
(defn- check-configuration-files
  ...
  (let [{:keys [project-config xdg-config]} config-status
        project-status (cond
                         (not (:exists project-config))
                         {:level :info
                          :message (str "Project config " (:path project-config) " (not found)")}

                         (not (:readable project-config))
                         {:level :error
                          :message (str "Project config " (:path project-config) " (not readable)")}

                         :else
                         {:level :ok
                          :message (str "Project config " (:path project-config) " (loaded)")})

        xdg-status (cond
                     (not (:exists xdg-config))
                     {:level :info
                      :message (str "User config " (:path xdg-config) " (not found)")}

                     (not (:readable xdg-config))
                     {:level :error
                      :message (str "User config " (:path xdg-config) " (not readable)")}

                     :else
                     {:level :ok
                      :message (str "User config " (:path xdg-config) " (loaded)")})]
...
(defn- check-required-configuration
  "Check that required configuration values are present in final loaded config.
   Returns a vector of maps with :level :category :message keys."
  [config]
  (let [missing-values (cond-> []
                         (not (get-in config [:jira :base-url]))
                         (conj "jira.base-url")

                         (not (get-in config [:jira :username]))
                         (conj "jira.username")

                         (not (get-in config [:jira :api-token]))
                         (conj "jira.api-token")

                         (not (get-in config [:workspace :root-dir]))
                         (conj "workspace.root-dir")
        ...
        message (if (empty? missing-values)
                  ...
                  (str "Missing required config: " (str/join ", " missing-values) " (add to config file or set env vars)"))
...
(defn- check-workspace-directories
  ...
  (let [base-statuses [{:level :ok :category "Workspace" :message (str "Root " (:root-dir workspace-config))}
                       {:level :ok :category "Workspace" :message (str "Logs " (:logs-dir workspace-config))}
                       {:level :ok :category "Workspace" :message (str "Tickets " (:tickets-dir workspace-config))}
                       {:level :ok :category "Workspace" :message (str "Docs " (:docs-dir workspace-config))}]
...
(defn doctor-command
  ...
  (let [config (config/load-config)
        ...
        all-statuses (concat
                      ...
                      (check-workspace-directories workspace-config workspace-check)
