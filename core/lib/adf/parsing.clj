...
(defn- create-formatted-node
  ...
  (case (:type match)
    ...
    :url (nodes/create-text-node (:text match) [(nodes/create-link-mark (:href match))])))
    ...
    (let [;; Filter out separator lines (e.g., |---|---|)
          ...
          (map (fn [line-idx line]
                 (let [;; Split by | and clean up cells
                   ...
                   (if (= line-idx 0)
                     (nodes/create-table-row (map #(nodes/create-table-header %)
                                                  cleaned-cells))
   ...
   (fn [groups line]
     (let [line-data (classify-line line (last groups))
       ...
       (cond
         ...
         (= line-type :code-block-start)
         (conj groups {:type :code-block,
                       :language (:language line-data),
                       :content [],
                       :closed? false})
         (= line-type :code-block-content)
         (update-in groups [(dec (count groups)) :content]
                    conj (:line line-data))
         ...
         (case line-type
           :paragraph (update-in groups
                                 ...
                                 str " " (:content line-data))
           (:bullet-list :ordered-list :table-row)
           (update-in groups [(dec (count groups)) :lines]
                      conj (:line line-data))
         ...
         :else (case line-type
                 ...
                 :paragraph (conj groups {:type :paragraph,
                                          :content (:content line-data)})
                 (:bullet-list :ordered-list :table-row)
                 (conj groups {:type line-type,
                               :lines [(:line line-data)]})
                 :blockquote (conj groups {:type :blockquote,
                                           :content (:content line-data)})
