...
(defn enhance-content
  ...
  (when-not (map? ai-config)
    (throw (ex-info "AI config must be a map"
                    {:type :invalid-input, :value ai-config})))
  (when (:debug ai-config) (println "Enhancing content with AI..."))
              ...
              (cond
                ...
                (map? content-text)
                  (do (when (:debug ai-config)
                        ...
                        (println (json/generate-string content-text
                                                       {:pretty true})))
                ...
                :else (do (when (:debug ai-config)
                            (println
                              "No content received from AI - using original"))
