...
(defn parse-flags
  ...
  (let [process-flag
          (fn [{:keys [args flags remaining], :as acc} arg]
            (cond
              ...
              (= arg "--debug-jira") (-> acc
                                         ...
                                         (update :remaining rest))
              (= arg "--debug-sprint") (-> acc
                                           (assoc-in [:flags :debug-sprint]
                                                     true)
                                           (update :remaining rest))
              (= arg "--list-drafts") (-> acc
                                          (assoc-in [:flags :list-drafts] true)
                                          (update :remaining rest))
              (= arg "--clean-drafts") (-> acc
              ...
              (str/starts-with? arg "-") (update acc :remaining rest)
              ;; Regular arguments
              :else (-> acc
...
(defn dispatch-command
  ...
  (case command
    ...
    ("log" "l")
      (let [result (cond
                     ...
                     :else {:error :unknown-subcommand,
                            :message (str "Unknown log subcommand: "
                                          (first args)),
                            :context {:action :exit-with-error}})]
        ;; Return error for caller to handle
        (if (:error result) result {:success true, :result result}))
    "pipe" (try {:success true, :result (apply (:pipe command-registry) args)}
                (catch Exception e
                  {:error :command-failed,
                   :message (str "Pipe command failed: " (.getMessage e)),
                   :context {:command "pipe", :args args}}))
    ("start-day" "sd")
      (try {:success true, :result (daily-log/start-day-command)}
           (catch Exception e
             {:error :command-failed,
    ...
    "inspect-ticket"
      (try {:success true, :result ((:inspect-ticket command-registry) args)}
           (catch Exception e
             {:error :command-failed,
              :message (str "Inspect ticket command failed: " (.getMessage e)),
              :context {:command "inspect-ticket", :args args}}))
    "doctor" (try {:success true, :result ((:doctor command-registry))}
                  (catch Exception e
                    {:error :command-failed,
                     :message (str "Doctor command failed: " (.getMessage e)),
                     :context {:command "doctor"}}))
    "init-prompt"
      (try {:success true, :result ((:init-prompt command-registry) args)}
           (catch Exception e
             {:error :command-failed,
    ...
    {:error :unknown-command,
     ...
     :context {:available-commands ["help" "log" "l" "pipe" "start-day" "sd"
                                    "quick-story" "qs" "inspect-ticket" "doctor"
                                    "init-prompt"],
               :action :show-help}}))
