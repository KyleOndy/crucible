...
(defn get-parent-command-java
  ...
  []
  (try (let [current-handle (java.lang.ProcessHandle/current)
...
(defn get-parent-command-ps
  ...
  []
  (try (let [current-pid (.pid (java.lang.ProcessHandle/current))
         ...
         (if-not (str/blank? ppid)
           ...
           {:success true,
            :result
              {:command nil, :raw-command nil, :method :ps, :reason :no-ppid}}))
       (catch Exception e
         {:error :ps-failed,
