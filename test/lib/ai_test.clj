...
(deftest test-parse-json-content
  ...
  (testing "Parses direct JSON strings"
    (let [json-string "{\"title\": \"Direct Title\", \"description\": \"Direct Desc\"}"
  ...
  (testing "Handles nil input"
    (let [result (ai/parse-json-content nil)]
      (is (nil? result)))))
...
(deftest test-extract-content
  (testing "Extracts and parses content from SMG provider format"
    (let [config {}
      ...
      (is (= (:description result) "Description goes here\nsometime newlines."))))
  ...
  (testing "Extracts and parses content from OpenRouter Linux response"
    (let [config {}
      ...
      (is (map? result))
      (is (= (:title result) "Increase webServer minimum replica count from current to 6 for traffic resilience"))
      (is (.contains (:description result) "Currently, the system's ability to handle traffic bursts"))
...
(deftest test-enhance-content-integration
  ...
  (testing "End-to-end enhancement with OpenAI response"
    (with-redefs [ai/call-ai-model (fn [_prompt _ai-config]
                                     {:success true
                                      :content (ai/extract-content openai-response-example {})})]
      (let [result (ai/enhance-content {:title "Original Title"
                                        :description "Original Desc"}
                                       {})]
        (is (map? result))
  ...
  (testing "Handles AI call failure gracefully"
    (with-redefs [ai/call-ai-model (fn [_prompt _ai-config]
                                     {:success false :error :timeout :message "Timeout occurred"})]
      (let [result (ai/enhance-content {:title "Original Title"
                                        :description "Original Desc"}
                                       {})]
        (is (map? result))
